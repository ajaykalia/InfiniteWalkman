...
def resample_features_from_point(track, feature):
    beats = getattr(track.analysis, 'beats')
    feature_vector = np.zeros((len(beats)))
    for i in xrange(len(beats)):
        if feature == 'loudness':
            feature_vector[i] = beats[i].mean_loudness()
        if feature == 'duration':
            feature_vector[i] = beats[i].duration
        if feature == 'confidence':
            feature_vector[i] = beats[i].confidence
    return feature_vector

def resample_features_from_vector(data, feature='timbre'):
    """
    ...
    segments = getattr(data.analysis, 'segments')
    markers = getattr(data.analysis, rate)
    ...
    mat = ret['matrix'] = np.zeros((len(tmp_markers), 12), dtype=np.float32)
